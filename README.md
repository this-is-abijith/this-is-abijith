<div align="center">

<!-- Animated Header -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:00ff88,50:00d4ff,100:7b2ff7&height=200&section=header&text=Abijith%20Binu&fontSize=60&fontColor=ffffff&fontAlignY=38&desc=AI%20Engineer%20%7C%20ML%20Researcher%20%7C%20Builder&descAlignY=58&descSize=18&animation=fadeIn" />

</div>

---

<div align="center">

**`MSc Artificial Intelligence · Mahatma Gandhi University`**

[![Portfolio](https://img.shields.io/badge/🌐_Portfolio-abijith--ai.vercel.app-00ff88?style=for-the-badge&labelColor=0d0d0d)](https://abijith-ai.vercel.app)
[![GitHub](https://img.shields.io/badge/GitHub-this--is--abijith-ffffff?style=for-the-badge&logo=github&labelColor=0d0d0d)](https://github.com/this-is-abijith)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-abijith--binu-0077B5?style=for-the-badge&logo=linkedin&labelColor=0d0d0d)](https://linkedin.com/in/abijith-binu)
[![Email](https://img.shields.io/badge/Email-abijithbinu654%40gmail.com-ea4335?style=for-the-badge&logo=gmail&labelColor=0d0d0d)](mailto:abijithbinu654@gmail.com)

</div>

---

## ◈ About Me

```python
class AbijithBinu:
    def __init__(self):
        self.role        = "AI/ML Engineer & Graduate Researcher"
        self.education   = "MSc Artificial Intelligence @ Mahatma Gandhi University"
        self.location    = "Kerala, India 🇮🇳"
        self.focus       = ["Computer Vision", "Deep Learning", "NLP", "MLOps"]
        self.building    = "Production-ready AI systems with clean web interfaces"
        self.philosophy  = "Ship fast. Learn faster. Build things that matter."

    def current_work(self):
        return [
            "🔬 Graduate Researcher @ Mahatma Gandhi University",
            "🧪 AI/ML Trainee @ Linnk Academy India, Kakkanad",
            "🚀 Building ML projects with Flask, Streamlit & React frontends",
        ]
```

---

## ◈ Featured Projects

<table>
<tr>
<td width="50%" valign="top">

### 🧠 Brain Tumor Detection
> Deep learning model for MRI scan classification with a production web interface.

`PyTorch` `CNN` `Flask` `Medical AI`

</td>
<td width="50%" valign="top">

### ♻️ Smart Waste Classification
> ResNet18 transfer learning on TrashNet dataset — **~78% val accuracy**, Streamlit UI.

`ResNet18` `Transfer Learning` `Streamlit` `PyTorch`

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 🌧️ Rainfall Prediction App
> Random Forest + Gradient Boosting ensemble (~83% accuracy) with animated dark-themed Flask UI.

`Scikit-learn` `Ensemble ML` `Flask` `Feature Engineering`

</td>
<td width="50%" valign="top">

### 🖼️ AI Image Upscaler
> Real-ESRGAN powered super-resolution for high-quality image enhancement.

`Real-ESRGAN` `Computer Vision` `Super Resolution`

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 🤚 Hand Gesture Particle System
> Real-time particle effects driven by hand landmarks using MediaPipe + canvas rendering.

`MediaPipe` `JavaScript` `Computer Vision` `WebGL`

</td>
<td width="50%" valign="top">

### 🎬 Multimodal Video Summarizer
> Extracts audio + visual context from video to generate intelligent summaries.

`Multimodal AI` `NLP` `Video Processing` `Transformers`

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 🚗 Number Plate Detection
> Automated license plate recognition system using computer vision.

`OpenCV` `Object Detection` `Python`

</td>
<td width="50%" valign="top">

### 🦾 Iron Man AR Suit-Up
> Real-time webcam AR suit-up experience using MediaPipe Hands & Pose — single-file HTML.

`MediaPipe` `Pose Estimation` `AR` `HTML/CSS/JS`

</td>
</tr>
</table>

---

## ◈ Tech Stack

<div align="center">

**Languages & Core**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)

**AI / ML / DL**

![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=flat-square&logo=tensorflow&logoColor=white)
![scikit-learn](https://img.shields.io/badge/Scikit--Learn-F7931E?style=flat-square&logo=scikitlearn&logoColor=white)
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?style=flat-square&logo=opencv&logoColor=white)
![MediaPipe](https://img.shields.io/badge/MediaPipe-00897B?style=flat-square&logo=google&logoColor=white)
![HuggingFace](https://img.shields.io/badge/🤗_Transformers-FFD21E?style=flat-square)

**Web & Deployment**

![Flask](https://img.shields.io/badge/Flask-000000?style=flat-square&logo=flask&logoColor=white)
![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=flat-square&logo=streamlit&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=flat-square&logo=react&logoColor=61DAFB)
![Vercel](https://img.shields.io/badge/Vercel-000000?style=flat-square&logo=vercel&logoColor=white)

**Data & Tools**

![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat-square&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat-square&logo=numpy&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=flat-square&logo=jupyter&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![Google Colab](https://img.shields.io/badge/Colab-F9AB00?style=flat-square&logo=googlecolab&logoColor=black)

</div>

---

## ◈ GitHub Stats

<div align="center">
<img height="160" src="https://github-readme-stats.vercel.app/api?username=this-is-abijith&show_icons=true&theme=chartreuse-dark&hide_border=true&bg_color=0d0d0d&title_color=00ff88&icon_color=00d4ff&text_color=ffffff&count_private=true" />
<img height="160" src="https://github-readme-stats.vercel.app/api/top-langs/?username=this-is-abijith&layout=compact&theme=chartreuse-dark&hide_border=true&bg_color=0d0d0d&title_color=00ff88&text_color=ffffff" />
</div>

<div align="center">
<img src="https://github-readme-streak-stats.herokuapp.com/?user=this-is-abijith&theme=dark&hide_border=true&background=0d0d0d&stroke=00ff88&ring=00d4ff&fire=7b2ff7&currStreakLabel=00ff88&sideLabels=ffffff&dates=888888" />
</div>

---

## ◈ What I'm Working On

```
▸ Building production-grade ML systems with clean, interactive web UIs
▸ Exploring multimodal AI (vision + language + audio)
▸ Wrapping ML projects as deployable Telegram bots & REST APIs
▸ Research in deep learning — transfer learning & model optimization
```

---

<div align="center">

<!-- Snake animation — requires GitHub Actions setup in your repo -->
<img src="https://raw.githubusercontent.com/this-is-abijith/this-is-abijith/output/github-contribution-grid-snake-dark.svg" alt="contribution snake" />

</div>

<div align="center">

![visitors](https://visitor-badge.laobi.icu/badge?page_id=this-is-abijith.this-is-abijith&color=00ff88)

*"Build the things you wish existed."*

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:7b2ff7,50:00d4ff,100:00ff88&height=100&section=footer" />

</div>
